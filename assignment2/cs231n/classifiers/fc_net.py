...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        f1, aff1_cache = affine_forward(X, W1, b1)
        f2, relu_cache = relu_forward(f1)
        scores, aff2_cache = affine_forward(f2, W2, b2)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        dx2, grads['W2'], grads['b2'] = affine_backward(grad, aff2_cache)
        dx2 = relu_backward(dx2, relu_cache)
        dx1, grads['W1'], grads['b1'] = affine_backward(dx2, aff1_cache)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        ############################################################################
        loss, grad = softmax_loss(scores, y)
        dx, dw, db = affine_backward(grad, _cache['aff{}'.format(self.num_layers)])
        ...
        if self.reg > 0:
            ...
            for layer in range(1, self.num_layers + 1):
                grads['W{}'.format(layer)] += self.reg * self.params['W{}'.format(layer)]
                _sum += np.sum(self.params['W{}'.format(layer)] ** 2)
            loss += 0.5 * self.reg * _sum
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
